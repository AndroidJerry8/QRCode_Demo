...
package com.jerry.zxing.library.camera;
    ...
    public synchronized Rect getFramingRect() {
        if (framingRect == null) {
            ...
            int width = findDesiredDimensionInRange(screenResolution.x,
                    MIN_FRAME_WIDTH, MAX_FRAME_WIDTH) * 3 / 5;
            int height = findDesiredDimensionInRange(screenResolution.y,
                    MIN_FRAME_HEIGHT, MAX_FRAME_HEIGHT) * 3 / 5;

            int leftOffset = (screenResolution.x - width) / 2;  //偏左
            int topOffset = (screenResolution.y - height) / 2;  //偏上
            //为了保证扫一扫的框是正方形，传的参数都是 width
